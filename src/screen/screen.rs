...
pub struct Board {
    field: [[CellStatus; CELL_HEIGHT]; CELL_WIDTH],
    nexts: [Mino; 5]
}

pub fn get_current_board(screen_number: usize) -> Board {
    // 画面取得
    ...
    // 盤面を取得する
    let mut field = [[CellStatus::UNKNOWN; CELL_HEIGHT]; CELL_WIDTH];
    ...
    return Board{field: field, nexts: nexts};
}
...
fn get_field_info(original: &Mat, image: &Mat, cell_size: &(usize, usize)) -> [[CellStatus; CELL_HEIGHT]; CELL_WIDTH] {
    let mut field = [[CellStatus::UNKNOWN; CELL_HEIGHT]; CELL_WIDTH];
    ...
    for y in 0..CELL_HEIGHT {
        for x in 0..CELL_WIDTH {
            ...
            if mean_hsv.2 >= 100 {
                field[x][y] = CellStatus::EXIST;
            } else {
                field[x][y] = CellStatus::NONE;
            };
        }
    }
    return field;
